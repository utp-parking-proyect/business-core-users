...
import com.utp.portal.mapper.CampusMapperImpl;
import com.utp.portal.mapper.RoleMapperImpl;
import com.utp.portal.mapper.UserMapperImpl;
...
class UserServiceImplTest {
  ...
  @Test
  void findByRole_returnsMatchingUsers_filteredByCampus() {
    User user = sampleUser();

    when(userRepository.findAllByRoleNameAndOptionalCampus("ROLE_SAE", 10L)).thenReturn(Flux.just(user));
    when(userRoleRepository.findAllByIdUser(1L)).thenReturn(Flux.empty());
    when(campusRepository.findById(10L)).thenReturn(Mono.empty());

    StepVerifier.create(userService.findByRole("ROLE_SAE", 10L))
        .assertNext(dto -> {
          assert dto.getIdUser().equals(1L);
        })
        .verifyComplete();
  ...
  @Test
  void findByRole_completesEmpty_whenNoUsersMatch() {
    when(userRepository.findAllByRoleNameAndOptionalCampus("ROLE_SAE", 99L)).thenReturn(Flux.empty());

    StepVerifier.create(userService.findByRole("ROLE_SAE", 99L))
        .verifyComplete();
  }
